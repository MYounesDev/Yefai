  ...
  return (
    ...
    >
      ...
      {!sidebarCollapsed && currentOrg && (
        <div className="px-3 py-2 border-b border-border">
          <button
            ...
            className={cn(
              'w-full flex items-center gap-2 p-2 rounded-lg hover:bg-surface-2 transition-colors text-left',
              organizations.length > 1 && 'cursor-pointer'
            )}
          >
            <div className="w-7 h-7 rounded-md bg-gradient-to-br from-violet-500 to-cyan-600 flex items-center justify-center shrink-0">
              <span className="text-white font-bold text-xs">{currentOrg.org_name[0]}</span>
            </div>
          ...
          <AnimatePresence>
            {orgDropdownOpen && (
              <motion.div
                ...
                exit={{ opacity: 0, y: -4 }}
                className="mt-1 rounded-lg border border-border bg-surface-2 overflow-hidden"
              >
      ...
      <div className="p-2 border-t border-border shrink-0">
        <div className={cn('flex items-center gap-2 px-2 py-1.5', sidebarCollapsed && 'justify-center')}>
