...
\begin{figure*}[t]
  \centering
  \shorthandoff{=}
  \includegraphics[width=0.98\textwidth]{images/diagram.png}
...
\toprule
Gereksinim grubu & Açıklama \\
\midrule
Veri seti yönetimi & Ayrıştırma, kalite raporu, set bazlı ayrım ve sentetik yedek parça verisi üretildi. \\
Görüntü çıkarımı & Eğitim ve embedding hattı uygulanmış; canlı yerel çıkarım dış kapıya bağlıdır. \\
Gerçek zamanlı akış & Kullanıcı arayüzü ve canlı gösterim kapsamına bırakılmıştır. \\
RAG sorgulama & Embedding ve hibrit arama bileşenleri doğal dil sorgulama için temel oluşturur. \\
Bildirim otomasyonu & Payload, tekrar deneme, yedek davranış ve kayıt sözleşmeleri doğrulanmıştır. \\
Masaüstü uygulama & Yerel kabuk ve canlı kullanıcı deneyimi arka uç sonrası ele alınacaktır. \\
Veritabanı ve embedding & Üst veri ve vektör temsilleri için yönetilen ilişkisel model planlanmıştır. \\
Yedek parça krizi & Sentetik katalog, risk skoru, satın alma hazırlığı ve alternatif tedarikçi mantığı oluşturulmuştur. \\
\bottomrule
\end{tabular}
\end{table}
